...
const AboutClient = ({ skills, experiences }: AboutClientProps) => {
  ...
  return (
    <>
      ...
      >
        <div
          className="flex justify-between h-full overflow-y-scroll scroll bg-gradient-radial from-gray-950 to-black"
          ref={containerRef}
        >
          <div className="p-4 md:px-12 md:py-10 lg:px-20 lg:py-16 xl:px-56 xl:py-40 flex flex-col gap-24 md:gap-32 lg:gap-48 xl:gap-64 w-full md:1/2">
            ...
            <div className="flex flex-col gap-32 justify-center" ref={skillRef}>
              ...
              >
                {skills.map((skill) => (
                  <div
                    className="rounded p-2 text-sm cursor-pointer bg-white text-black ring-1 ring-white hover:bg-black hover:text-white transition-all"
                    key={skill.id}
                  >
                    {skill.skill}
                  </div>
              ...
              {experiences.map((experience) => (
                <div
                  className={`flex justify-between w-full ${
                    experience.id % 2 === 0 && "flex-row-reverse"
                  }`}
                  key={experience.id}
                >
                  <div className="w-2/5 flex flex-col gap-2 ">
                    <h2 className="text-lg md:text-2xl font-semibold bg-gray-900 rounded-md p-1 md:p-4">
                      {experience.title}
                    </h2>
                    <p className="md:text-lg">{experience.description}</p>
